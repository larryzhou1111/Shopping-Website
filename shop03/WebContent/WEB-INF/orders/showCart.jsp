...
<table style="margin-top:10px;" width="999" align="center" cellpadding="0" cellspacing="0" class="thin-border">
	...
	<tr><td colspan="4">购物车中没有任何商品，请<a href="goods.do?method=list">选择商品</a></td></tr>
	</c:if>
	</tr>
	<c:forEach items="${shopCart.goodses}" var="sc">
	<c:set var="totalPrice" value="${totalPrice+sc.price }"></c:set>
	<tr>
	<td>${sc.goods.name }</td>
	<td>${sc.number }</td><td>${sc.price }</td>
	<td>
	<a href="orders.do?method=clearGoods&pid=${sc.goods.id }">清除</a>&nbsp;
	<a href="orders.do?method=goodsAddNumberInput&pid=${sc.pid }">添加数量</a>
	</td>
	</tr>
	</c:forEach>
